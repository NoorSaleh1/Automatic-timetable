...
class GeneticAlgorithm(object):
  ...
  def crossover_population(self, population):
    ...
    for idx in range(self.runTable.NUMB_OF_ELITE_SCHEDULES):
      _cross_over_popluation.schedules[idx] = deepcopy(population.schedules[idx])
    for idx in range(self.runTable.NUMB_OF_ELITE_SCHEDULES, len(population.schedules)):
      if self.runTable.CROSSOVER_RATE > get_random_number():
  ...
  def mutate_population(self, population):
    ...
    for idx in range(self.runTable.NUMB_OF_ELITE_SCHEDULES+1):
      _schedules[idx] = deepcopy(population.schedules[idx])
    
    for idx in range(self.runTable.NUMB_OF_ELITE_SCHEDULES+1, len(population.schedules)):
  ...
  def mutate_schedule(self, _mutate_schedule):
    ...
    for idx in range(len(_mutate_schedule.classes)):
      if self.runTable.MUTATION_RATE > get_random_number():
  ...
  def select_tournament_population(self, population):
    tournament_population = Population(size=self.runTable.TOURNAMENT_SELECTION_SIZE, data=self.data)
    for idx in range(self.runTable.TOURNAMENT_SELECTION_SIZE):
